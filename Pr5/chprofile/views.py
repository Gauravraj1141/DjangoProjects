...
from .forms import Registrationform, user_login_form, user_profile_change
...
def user_profile(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            formdata = user_profile_change(request.POST, instance=request.user)
            if formdata.is_valid():
                formdata.save()
                messages.success(request, "updated profile successfully!!")
        else:
            formdata = user_profile_change(instance=request.user)
        return render(request, "login/profile.html", {"name": request.user, "fm": formdata})
    else:
        return redirect("/login/")
